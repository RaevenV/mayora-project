...
function DataListContainerPPIC() {
  ...
  const handleApprovalToggle = async (id, currentApproval) => {
    try {
      await updateDoc(doc(db, "submittedProduct", id), {
        approvalPPIC: !currentApproval,
      });
      console.log("Approval toggled successfully!");
      setDataList((prevDataList) =>
        prevDataList.map((item) => {
          if (item.id === id) {
            return { ...item, approvalPPIC: !currentApproval };
          }
          return item;
        })
      );
          ...
          <div className="dataListItem-wrapper">
            ...
            <div className="dataList-item2">
              <button
                ...
                onClick={() => handleApprovalToggle(item.id, item.approvalPPIC)}
              >
                {item.approvalPPIC ? "Cancel" : "Approve"}
              </button>
            </div>
          </div>
        </div>
...
export default DataListContainerPPIC;
